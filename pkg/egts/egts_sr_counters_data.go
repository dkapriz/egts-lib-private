...
// SrCountersData структура под записи типа EGTS_SR_COUNTERS_DATA, которая используется аппаратно-программным
...
func (c *SrCountersData) Decode(content []byte, _ byte) error {
	var (
		err        error
		flags      byte
		counterVal []byte
...
func (c *SrCountersData) Encode(_ byte) ([]byte, error) {
	var (
		err    error
		flags  uint64
		result []byte
...
func (c *SrCountersData) Length(protocolVersion byte) uint16 {
	var result uint16

	if recBytes, err := c.Encode(protocolVersion); err != nil {
		result = uint16(0)
	} else {
