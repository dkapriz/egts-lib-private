...
// SrAdSensorsData структура под записи типа EGTS_SR_AD_SENSORS_DATA, которая применяется абонентским
...
func (e *SrAdSensorsData) Decode(content []byte, _ byte) error {
	var (
		err           error
		flags         byte
		analogSensVal []byte
...
func (e *SrAdSensorsData) Encode(_ byte) ([]byte, error) {
	var (
		err    error
		flags  uint64
		result []byte
...
func (e *SrAdSensorsData) Length(protocolVersion byte) uint16 {
	var result uint16
