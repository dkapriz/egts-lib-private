...
func TestEgtsSrAdSensorsData_Encode(t *testing.T) {
	sensDataBytes, err := testEgtsSrAdSensorsData.Encode(ProtocolVersionV1)
...
func TestEgtsSrAdSensorsData_Decode(t *testing.T) {
	...
	if assert.NoError(t, adSensData.Decode(srAdSensorsDataBytes, ProtocolVersionV1)) {
...
func TestEgtsSrAdSensorsDataaRs(t *testing.T) {
	...
	adSensDataRD := RecordDataSet{
		RecordData{
			...
			SubrecordLength: testEgtsSrAdSensorsData.Length(ProtocolVersionV1),
			SubrecordData:   &testEgtsSrAdSensorsData,
		},
	}
	testStruct := RecordDataSet{}

	testBytes, err := adSensDataRD.Encode(ProtocolVersionV1)
	if assert.NoError(t, err) {
		assert.Equal(t, testBytes, adSensDataRDBytes)

		if assert.NoError(t, testStruct.Decode(adSensDataRDBytes, ProtocolVersionV1)) {
