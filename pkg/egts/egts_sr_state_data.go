...
// SrStateData структура под записи типа EGTS_SR_STATE_DATA, которая используется для передачи на
...
func (e *SrStateData) Decode(content []byte, _ byte) error {
	var (
		err   error
		flags byte
	)
...
func (e *SrStateData) Encode(_ byte) ([]byte, error) {
	var (
		err    error
		flags  uint64
		result []byte
...
func (e *SrStateData) Length(protocolVersion byte) uint16 {
	var result uint16
