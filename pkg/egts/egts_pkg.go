...
func (p *Package) Decode(content []byte, opt ...func(*Options)) (uint8, error) {
	...
	if err = p.ServicesFrameData.Decode(dataFrameBytes, p.ProtocolVersion); err != nil {
		return egtsPcDecryptError, err
	}
...
// Encode кодирует структуру в байтовую строку
func (p *Package) Encode(opt ...func(*Options)) ([]byte, error) {
	...
	if p.ServicesFrameData != nil {
		sfrd, err = p.ServicesFrameData.Encode(p.ProtocolVersion)
		if err != nil {
			return result, err
		}
