...
func TestRecordDataSet_Encode_Version_1(t *testing.T) {
	...
	rdBytes, err := testRecordDataSet.Encode(ProtocolVersionV1)
	if assert.NoError(t, err) {
		assert.Equal(t, rdBytes, testRecordDataBytes)
	}
}

func TestRecordDataSet_Decode_Version_1(t *testing.T) {
	rds := RecordDataSet{}
	testRecordDataSet := RecordDataSet{
		RecordData{
	...
	if assert.NoError(t, rds.Decode(testRecordDataBytes, ProtocolVersionV1)) {
