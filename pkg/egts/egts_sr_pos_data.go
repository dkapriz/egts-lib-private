...
// SrPosData структура под записи типа EGTS_SR_POS_DATA, которая используется абонентским
...
type SrPosData struct {
	...
	NetworkIdentifier   uint32    `json:"NID"` //Protocol version 2
	LocalAreaCode       uint32    `json:"LAC"` //Protocol version 2
	CellIdentifier      int16     `json:"CID"` //Protocol version 2
	SignalStrength      byte      `json:"SS"`  //Protocol version 2
	Altitude            uint32    `json:"ALT"`
	SourceData          int16     `json:"SRCD"`
}

// Decode разбирает байты в структуру под записи
func (e *SrPosData) Decode(content []byte, protocolVersion byte) error {
	var (
		err   error
		flags byte
		speed uint64
	...
	if e.DigitalInputs, err = buf.ReadByte(); err != nil {
		return fmt.Errorf("не удалось получить битовые флаги, определяют состояние основных "+
			"дискретных входов: %v", err)
	}
	...
	tmpInt16Buf := make([]byte, 2)
	//Protocol version 2
	if protocolVersion >= 2 {
		bytesTmpBuf = []byte{0, 0, 0}
		if _, err = buf.Read(bytesTmpBuf); err != nil {
			return fmt.Errorf("не удалось получить идентификатор базовой станции: %v", err)
		}
		bytesTmpBuf = append(bytesTmpBuf, 0x00)
		e.NetworkIdentifier = binary.LittleEndian.Uint32(bytesTmpBuf)

		if _, err = buf.Read(tmpUint32Buf); err != nil {
			return fmt.Errorf("не удалось получить идентификатор локальной зоны базовой станции: %v", err)
		}
		e.LocalAreaCode = binary.LittleEndian.Uint32(tmpUint32Buf)

		if _, err = buf.Read(tmpInt16Buf); err != nil {
			return fmt.Errorf("не удалось получить идентификатор ячейки базовой станции: %v", err)
		}
		e.CellIdentifier = int16(big.NewInt(0).SetBytes(tmpInt16Buf).Int64())

		if e.SignalStrength, err = buf.ReadByte(); err != nil {
			return fmt.Errorf("не удалось получить уровень сигнала базовой станции: %v", err)
		}
	}
	...
	//if _, err = buf.Read(tmpInt16Buf); err != nil {
	//	return fmt.Errorf("не удалось получить данные, характеризующие источник (событие) из поля SRC: %v", err)
	//}
	//e.SourceData = int16(big.NewInt(0).SetBytes(tmpInt16Buf).Int64())

	return err
}

// Encode преобразовывает под запись в набор байт
func (e *SrPosData) Encode(protocolVersion byte) ([]byte, error) {
	var (
		err    error
		flags  uint64
		result []byte
	...
	if err = binary.Write(buf, binary.LittleEndian, e.DigitalInputs); err != nil {
		return result, fmt.Errorf("не удалось записать битовые флаги, определяют состояние основных "+
			"дискретных входов: %v", err)
	}
	...
	if protocolVersion >= 2 {
		...
		if _, err = buf.Write(bytesTmpBuf[:3]); err != nil {
			return result, fmt.Errorf("не удалось записать идентификатор базовой станции: %v", err)
		}

		binary.LittleEndian.PutUint32(bytesTmpBuf, e.LocalAreaCode)
		if _, err = buf.Write(bytesTmpBuf); err != nil {
			return result, fmt.Errorf("не удалось записать идентификатор локальной зоны базовой станции: %v", err)
		}

		cellIdentifier := []byte{uint8(e.CellIdentifier >> 8), uint8(e.CellIdentifier & 0xff)}
		if _, err = buf.Write(cellIdentifier); err != nil {
		...
		if err = binary.Write(buf, binary.LittleEndian, e.SignalStrength); err != nil {
			return result, fmt.Errorf("не удалось записать уровень сигнала базовой станции: %v", err)
		}
	}
	...
	//sourceData := []byte{uint8(e.SourceData >> 8), uint8(e.SourceData & 0xff)}
	//if _, err = buf.Write(sourceData); err != nil {
	//	return result, fmt.Errorf("не удалось записать данные, характеризующие источник (событие) из поля SRC: %v", err)
	//}
...
func (e *SrPosData) Length(protocolVersion byte) uint16 {
	var result uint16
