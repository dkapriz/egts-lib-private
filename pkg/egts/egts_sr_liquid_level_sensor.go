...
// SrLiquidLevelSensor структура под записи типа EGTS_SR_LIQUID_LEVEL_SENSOR, которая применяется
...
func (e *SrLiquidLevelSensor) Decode(content []byte, _ byte) error {
	var (
		err     error
		flags   byte
		sensNum uint64
...
func (e *SrLiquidLevelSensor) Encode(_ byte) ([]byte, error) {
	var (
		err    error
		flags  uint64
		result []byte
...
func (e *SrLiquidLevelSensor) Length(protocolVersion byte) uint16 {
	var result uint16
