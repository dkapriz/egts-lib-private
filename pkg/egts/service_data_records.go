...
func (s *ServiceDataSet) Decode(serviceDS []byte, protocolVersion byte) error {
	var (
		err   error
		flags byte
	)
	...
	for buf.Len() > 0 {
		...
		if sdr.TimeFieldExists == "1" {
			...
			if _, err := buf.Read(tm); err != nil {
				return fmt.Errorf("не удалось получить время формирования записи на стороне отправителя "+
					"SDR: %v", err)
			}
		...
		if buf.Len() != 0 {
			...
			if err = rds.Decode(rdsBytes, protocolVersion); err != nil {
				return err
			}
...
func (s *ServiceDataSet) Encode(protocolVersion byte) ([]byte, error) {
	var (
		result []byte
		flags  uint64
	)
	...
	for _, sdr := range *s {
		rd, err := sdr.RecordDataSet.Encode(protocolVersion)
		if err != nil {
			return result, err
		}
		...
		if sdr.TimeFieldExists == "1" {
			...
			if err := binary.Write(buf, binary.LittleEndian, tm); err != nil {
				return result, fmt.Errorf("не удалось записать время формирования записи на стороне отправителя "+
					"SDR: %v", err)
			}
		}
...
func (s *ServiceDataSet) Length(protocolVersion byte) uint16 {
	var result uint16
