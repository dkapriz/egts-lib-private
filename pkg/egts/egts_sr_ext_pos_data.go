...
// SrExtPosData структура под записи типа EGTS_SR_EXT_POS_DATA, которая используется абонентским
...
func (e *SrExtPosData) Decode(content []byte, _ byte) error {
	var (
		err   error
		flags byte
	)
...
func (e *SrExtPosData) Encode(_ byte) ([]byte, error) {
	var (
		err    error
		flags  uint64
		result []byte
...
func (e *SrExtPosData) Length(protocolVersion byte) uint16 {
	var result uint16
