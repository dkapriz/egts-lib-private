...
var (
	egtsPkgPosDataBytesV1 = []byte{0x01, 0x00, 0x03, 0x0B, 0x00, 0x23, 0x00, 0x8A, 0x00, 0x01, 0x49, 0x18, 0x00, 0x61,
...
func TestEgtsPackagePosData_Encode(t *testing.T) {
	...
	if assert.NoError(t, err) {
		assert.Equal(t, posDataBytes, egtsPkgPosDataBytesV1)
	}
}
...
func TestEgtsPackagePosData_Decode(t *testing.T) {
	...
	if _, err := egtsPkg.Decode(egtsPkgPosDataBytesV1); assert.NoError(t, err) {
...
func TestFullCycleCoding(t *testing.T) {
	...
	_, err := egtsPkg.Decode(egtsPkgPosDataBytesV1)
	if assert.NoError(t, err) {
		assert.Equal(t, egtsPkg, egtsPkgPosData)
