...
func TestStorageRecord_Encode(t *testing.T) {
	egtsPlusBytes, err := testEgtsPlusData.Encode(ProtocolVersionV1)
...
func TestStorageRecord_Decode(t *testing.T) {
	...
	if assert.NoError(t, egtsPlus.Decode(srEgtsPlusBytes, ProtocolVersionV1)) {
...
func TestStorageRecordRs(t *testing.T) {
	...
	egtsPlusDataRD := RecordDataSet{
		RecordData{
			...
			SubrecordLength: testEgtsPlusData.Length(ProtocolVersionV1),
			SubrecordData:   &testEgtsPlusData,
		},
	}
	testStruct := RecordDataSet{}

	testBytes, err := egtsPlusDataRD.Encode(ProtocolVersionV1)
	if assert.NoError(t, err) {
		assert.Equal(t, testBytes, egtsPlusRDBytes)

		if assert.NoError(t, testStruct.Decode(egtsPlusRDBytes, ProtocolVersionV1)) {
