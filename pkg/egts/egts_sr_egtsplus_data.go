...
func (m *StorageRecord) Decode(content []byte, protocolVersion byte) error {
	return proto.Unmarshal(content, m)
}

func (m *StorageRecord) Encode(protocolVersion byte) ([]byte, error) {
	return proto.Marshal(m)
}

func (m *StorageRecord) Length(protocolVersion byte) uint16 {
	var result uint16

	if recBytes, err := m.Encode(protocolVersion); err != nil {
		result = uint16(0)
	} else {
