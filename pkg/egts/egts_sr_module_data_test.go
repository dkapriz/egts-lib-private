...
var (
	...
	//nolint:lll
	testSrModuleDataBytes = []byte{0x1, 0x0, 0x0, 0x0, 0x0, 0x1, 0x1, 0x3, 0x1, 0x1, 0x1, 0x54, 0x42, 0x31, 0x30,
		0x31, 0x31, 0x30, 0x31, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x31, 0x30, 0x32, 0x32, 0x30, 0x32, 0x33, 0x0,
		0x72, 0x65, 0x76, 0x31, 0x37, 0x30, 0x31, 0x32, 0x32, 0x2e, 0x41, 0x0}
)

// Проверяем, что рекордсет работает правильно с данным типом пд записи
//
//nolint:dupl
func TestEgtsSrModuleDataRs(t *testing.T) {
	...
	testBytes, err := stateDataRD.Encode(ProtocolVersionV1)
	if assert.NoError(t, err) {
		assert.Equal(t, testBytes, stateDataRDBytes)

		if assert.NoError(t, testStruct.Decode(stateDataRDBytes, ProtocolVersionV1)) {
...
func TestSrModuleData_Decode(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields(testSrModuleData),
			//nolint:lll
			args: args{content: []byte{0x01, 0x0, 0x0, 0x0, 0x0, 0x1, 0x1, 0x3, 0x1, 0x1, 0x1, 0x54, 0x42, 0x31,
				0x30, 0x31, 0x31, 0x30, 0x31, 0x30, 0x30, 0x30, 0x30, 0x30, 0x30, 0x31, 0x30, 0x32, 0x32, 0x30,
				0x32, 0x33, 0x00}},
			wantErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := e.Decode(tt.args.content, ProtocolVersionV1); (err != nil) != tt.wantErr {
...
func TestSrModuleData_Encode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := e.Encode(ProtocolVersionV1)
...
func TestSrModuleData_Length(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := e.Length(ProtocolVersionV1); got != tt.want {
